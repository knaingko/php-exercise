...
<?php
    ...
    {
        $Id = $_GET['id'];
        $data = GetDataByKey('.././data/month.csv','MonthID',$Id);
        $Name = $data['MonthName'];
    }
    if(!empty($_POST))
    {
        ...
        $data = array('MonthID'=>$Id, 'MonthName'=>$Name);

        if(UpdateDataByKey('.././data/month.csv','MonthID',$Id,$data)){
...
<section id="breadcrumb" class="container">
    ...
    echo(display_breadcrumb(
        array(
            ...
            array("label"=>"Month Information","url"=>"country-list.php","active"=>"false"),
            array("label"=>"Month Edit","url"=>"country-edit.php","active"=>"true")
        )
    ));
    ?>
</section>
...
<div class="container">
  <div class="row">
    <div class="col-xs-offset-2 col-sm-8 col-md-8 form_table  animated zoomIn">
      <h2>Month Edit Form</h2>
        <form role="form" id="MonthForm" name="MonthForm">
          <table class="table table-borderless">
            <tr>
              <td>Month ID:</td>
              <td><input type="text" class="form-control input-sm" name="month_id" id="month_id" value="<?php echo($Id);?>" placeholder="Enter month id" required /></td>
            </tr>
            <tr>
              <td>Month Name:</td>
              <td><input type="text" class="form-control input-sm" name="month_name" id="month_name" value="<?php echo($Name);?>" placeholder="Enter month name" required /></td>
            </tr>                    
            <tr>
              <td></td>
              <td><button type="button" class="save" onclick="SaveData();">Save</button>
                  <button type="button" class="save" onclick="window.open('month-list.php','_self');">Cancel</button>
              </td>
            </tr>
          </table>
        </form>
