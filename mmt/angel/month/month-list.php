...
<section id="breadcrumb" class="container">
  ...
  echo (display_breadcrumb(
      array(
          ...
          array("label"=>"Country Information","url"=>"month-list.php","active"=>"true")
      ))
  );
  ?>
</section>
...
<div class="container">
   <div class="row"> 
        <div class="col-xs-offset-1 col-sm-10 col-md-10 form_table"> 
            ...
            <div id="myDIV">
            <form id="MonthList" name="MonthList" class="new-form">
                <table class="table table-bordered">
                    ...
                    <?php $i = 1; foreach ($data as $value) { ?>
                      ...
                      <tr>
                        <td class="check" ><label><input type="checkbox" id="delCheck[]" name="delCheck[]" value="<?php echo $value["MonthID"] ?>"/>&nbsp;<?php echo($i);?></label></td>
                        <td><?php echo($value['MonthID']); ?></td>
                        <td><?php echo($value['MonthName']); ?></td>
                        <td class="check"><button type="button" class="btn edit" onclick="show_edit('<?php echo( $value["MonthID"] ) ?>')">Edit</button></td>
                      </tr>
...
<script lang="javascript" type="text/javascript">
        ...
        function delete_all()
        {
            with(document.MonthList)
            {
                hidCode.value = '';
                hidDelete.value = 'multi';
                action = 'month-list.php';
                method="post";
                submit();
            }
        }
        ...
        {
            window.open('month-edit.php?id='+ id,'_self');
        }

        function show_entry()
        {
            window.open('month-entry.php','_self');
        }
