...
<?php
    ...
    {
        $Id = $_GET['id'];
        $data = GetDataByKey('.././data/category.csv','CategoryID',$Id);
    ...
    if(!empty($_POST))
    {
        ...
        $data = array('CategoryID'=>$Id, 'CategoryName'=>$Name);

        if(UpdateDataByKey('.././data/category.csv','CategoryID',$Id,$data)){
...
<section id="breadcrumb" class="container">
    ...
    echo(display_breadcrumb(
        array(
            ...
            array("label"=>"Category Edit","url"=>"category-edit.php","active"=>"true")
        )
    ));
    ?>
</section>
      ...
      <h2>Category Edit Form</h2>
        <form role="form" id="CategoryForm" name="CategoryForm">
          <table class="table table-borderless">
            <tr>
              <td>Category ID:</td>
              <td><input type="text" class="form-control input-sm" name="category_id" id="category_id" value="<?php echo($Id);?>" placeholder="Enter category id" required /></td>
            </tr>
            <tr>
              <td>Category Name:</td>
              <td><input type="text" class="form-control input-sm" name="category_name" id="category_name" value="<?php echo($Name);?>" placeholder="Enter category name" required /></td>
            </tr>                    
            <tr>
              <td></td>
              <td><button type="button" class="save" onclick="SaveData();">Save</button>
                  <button type="button" class="save" onclick="window.open('category-list.php','_self');">Cancel</button>
              </td>
            </tr>
          </table>
        </form>
